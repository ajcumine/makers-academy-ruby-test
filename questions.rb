...
def remove_nils_and_false_from_array(array)
  array.compact.reject {|e| e == false }
end
...
def array_sort_by_last_letter_of_word(array)
  array.map { |e| e.reverse}.sort.map {|e| e.reverse}
end
...
def number_of_elements_that_are_palindromes(array)
  array.map { |e| e == e.reverse}.count(true)
end

# return the shortest word in an array
def shortest_word_in_array(array)
  a = array.map{|e| e.length}
  b = a.sort
  array.fetch(a.index(b[0]))
end
...
def titleize_a_string(string)
  s = string.split(' ').map{ |s| s == 'the' || s == 'a' || s == 'and' ? s : s.capitalize}
  s[0] = s[0].capitalize
  s.join(' ')
end
...
def is_a_2014_bank_holiday?(date)
  bank_holidays = ['18/4', '21/4', '5/5', '26/5', '25/8', '25/12', '26/12']
